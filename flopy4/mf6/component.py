...
class Component(DimensionResolverMixin, ABC, MutableMapping):
    ...
    def _children(self) -> "dict[str, Component]":
        ...
        `.name` resolution and parent-side attachment is handled
        explicitly by `_set_child_parents()`/`__attrs_post_init__` (see
        their docstrings) -- this property only reads the result, it
        doesn't drive either mechanism.
    ...
    def write(self, format: str = MF6, context: Optional[WriteContext] = None) -> None:
        ...
        context : WriteContext, optional
            ...
            or default settings.
        """
        # Re-resolve child names/parents before writing: covers a child
        ...
        # `_set_child_parents()` on its own. Idempotent for already-named
        # children (see `_resolve_child_name`'s `_is_default_child_name`
        # check), so safe to always re-run here. Without this, siblings
        # left at their shared class-name default collide on `.name` --
        # used as the MF6 PNAME in the namefile (`Binding.from_component()`)
        # -- which some MF6 builds tolerate and others don't.
        self._set_child_parents()
